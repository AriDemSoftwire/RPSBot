!(function (e) {
    ...
    function (e, t, r) {
        const { BotError: o } = r(0),
            s = ["R", "P", "S", "W", "D"];
        e.exports = class {
            constructor(e, t, r, o) {
                (this.instanceIds = { 1: e[0], 2: e[1] }),
                    (this.score = { 1: 0, 2: 0 }),
                    (this.dynamite = { 1: o.dynamite, 2: o.dynamite }),
                    (this.gamestate = { 1: { rounds: [] }, 2: { rounds: [] } }),
                    (this.scoreToWin = o.scoreToWin),
                    (this.roundLimit = o.roundLimit),
                    (this.runnerClient1 = t),
                    (this.runnerClient2 = r),
                    (this.nextRoundPoints = 1);
            }
            updateDynamite(e) {
                for (let t = 1; t <= 2; t++)
                    if (
                        ("D" === e[t] && (this.dynamite[t] -= 1),
                        this.dynamite[t] < 0)
                    )
                        throw new o(t, "dynamite");
            }
            updateGamestate(e) {
                this.gamestate[1].rounds.push({ p1: e[1], p2: e[2] }),
                    this.gamestate[2].rounds.push({ p1: e[2], p2: e[1] });
            }
            updateScore(e) {
            ...
            getOutput(e, t) {
                ...
                return (
                    t &&
                        t.errorPlayer &&
                        ((r.errorBot = t.errorPlayer),
                        (r.errorReason = t.errorReason),
                        (r.errorStack = t.stack),
                        (r.winner = 3 - t.errorPlayer)),
                    r
                );
            }
            play() {
                return this.scoreToWin <= Math.max(this.score[1], this.score[2])
                    ? this.getOutput("score")
                    : this.gamestate[1].rounds.length >= this.roundLimit
                    ? this.getOutput("round limit")
                    : Promise.all([
                          this.runnerClient1
                              .makeMove(this.instanceIds[1], this.gamestate[1])
                              .catch((e) => this.handleBotError(e, 1)),
                          this.runnerClient2
                              .makeMove(this.instanceIds[2], this.gamestate[2])
                              .catch((e) => this.handleBotError(e, 2)),
                      ])
                          .then((e) => {
                              const t = { 1: e[0], 2: e[1] };
                              this.updateGamestate(t),
                                  this.updateDynamite(t),
                                  this.updateScore(t);
                          })
                          .then(() => this.play())
                          .catch((e) => this.getOutput("error", e));
            }
            handleBotError(e, t) {
    ...
    function (module, exports, __webpack_require__) {
        ...
        ((process.argv[4] && isNaN(process.argv[4])) ||
            (process.argv[5] && isNaN(process.argv[5])) ||
            (process.argv[6] && isNaN(process.argv[6])) ||
            process.argv.length < 4) &&
            (console.log("Specify 2 arguments with the file path to the bots:"),
            console.log("\n\tnode dynamite-cli.js myBot1.js myBot2.js\n"),
            console.log(
                "You may also optionally specify the number of matches, score to win, and number of dynamite (in that order)"
            ),
            console.log(
        ...
        function playGames(e) {
            e > 0 &&
                play(1, 2, cliRunnerClient, cliRunnerClient, options, console)
                    .then((t) => {
                        console.log(`Game ${options.games - e + 1} results:`),
                            console.log(`Winner: p${t.winner}`),
                            console.log(`Score: ${t.score[1]} - ${t.score[2]}`),
                            console.log(`Reason: ${t.reason}`),
                            t.errorBot &&
                                console.log(
                                    `Player ${t.errorBot} failed:\n${t.errorStack}`
                                ),
                            playGames(e - 1);
                    })
